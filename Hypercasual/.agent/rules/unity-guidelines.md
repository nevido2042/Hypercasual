...
## 3. 트윈 관리 (DOTween Management)
- **트윈 생명 주기 관리:** 모든 트윈은 생성 시 반드시 `.SetLink(gameObject)`를 사용하여 오브젝트의 파괴 또는 비활성화 시 자동으로 정리되도록 한다.
- **안전한 트윈 재시작:** 동일한 트랜스폼이나 변수에 대해 연달아 애니메이션을 실행할 경우, 새로운 트윈을 시작하기 전 반드시 `DOKill()`을 호출하여 이전 트윈과의 충돌 및 메모리 누수를 방지한다.